...
function Tasks() {
  ...
  const getData=async()=> {
    ...
    try {
      const res=await axios.get(`${process.env.REACT_APP_SERVERURL}/tasks/${userId}`);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${process.env.REACT_APP_SERVERURL}/tasks/delete/${id}`);
  ...
  const handleComplete = async (id) => {
    try {
      await axios.patch(`${process.env.REACT_APP_SERVERURL}/tasks/done/${id}`, {
    ...
    <>
      ...
      <ul className="flex-col justify-between">
        {sortedTasks.map((task) => (
          <ReactCardFlip isFlipped={task.isCompleted} flipDirection="vertical">
            <li
              key={task.id}
